...
- 2026-09-15: Claude agregó la sección **Tarjetas** (`/tarjetas`, nuevo ítem de nav): tarjetas de crédito/débito y cuentas bancarias con nombre, tipo, entidad, últimos 4 dígitos y cupo (migración `0036_cards_details.sql`; nunca el número completo, vencimiento ni CVV — los últimos 4 dígitos sí se permiten, ver `CONVENCIONES.md`). `movements.card_id` (misma migración) vincula un movimiento a una tarjeta/cuenta, elegida a mano en el editor de Gastos ("Tarjeta / cuenta", igual que "Fijo asociado") o **detectada sola** por `widgetExpenses.js` a partir de "con tu T.Cred \*9317" / "desde tu cuenta \*5702" en el mensaje (si ninguna o más de una tarjeta activa coincide, el gasto igual se registra, solo sin tarjeta vinculada). La sección de tarjetas que vivía en Configuración se movió a `/tarjetas` (queda un link en Configuración). Smoke cubre CRUD de tarjetas (tipo/entidad/últimos_digitos/cupo, validaciones) y que los mensajes reales de compra (T.Cred) y QR/transferencia (cuenta) queden vinculados a la tarjeta/cuenta correcta.
...
- 2026-07-15: Histórico de fijos en Suscripciones incorpora filtro Todos/Ingresos/Gastos y resumen anual calculado por backend (ingresos, gastos y balance); ampliación completada y publicada.
...
- 2026-07-15: la bandeja de Notificaciones consulta únicamente hoy y ayer mediante `GET /api/notifications?desde=`, muestra inicialmente 5 avisos y revela bloques adicionales de 5 con `Mostrar más`; publicado como `d595f491-6178-4599-b0ea-155bc5926f5d`.
- 2026-07-15: la configuración general de Notificaciones, incluido Pushover, queda oculta en un modal con desplazamiento propio abierto desde el engranaje junto al título; publicado como `a3f51148-4f99-4787-896b-13d23314b502`.
...
- La suscripción móvil de Eventos usa `CALENDAR_FEED_TOKEN`: la UI autenticada obtiene la URL en `/api/events/calendar-url`; el único bypass de Access permitido es la ruta exacta `/api/events/calendar.ics`, que el Worker protege con el token.
- En producción existe la aplicación Access `Calendario móvil Nibor` para esa ruta exacta. Verificado: sin token/incorrecto responde 404, con token responde `200 text/calendar`, `/api/events` continúa detrás del login y la UI móvil 390 px no tiene overflow.
...
- `reminders` guarda Recordatorios. La regla `recordatorios` (claves `regla/push/prioridad/sonido_recordatorios`, seed en 0028) avisa el día programado y reinsiste VARIAS VECES AL DÍA — una por franja: cada recordatorio puede tener `repetir_horas` propio (migración 0030) y si no usa el general `recordatorios_repetir_horas` (seed 0029, default 4) — con dedupe `rec:{id}:{fecha}:{franja}`, sin generar avisos dentro del horario de silencio, hasta marcar hecho; con `frecuencia_dias`, completar reprograma `proxima_fecha` desde hoy y calla el resto del día. Si un recordatorio tiene `hora`, el aviso del día programado espera a esa hora.

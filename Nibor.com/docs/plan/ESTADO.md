...
- `reminders` guarda Recordatorios. La regla `recordatorios` (claves `regla/push/prioridad/sonido_recordatorios`, seed en 0028) avisa el día programado y reinsiste VARIAS VECES AL DÍA — una por franja de `recordatorios_repetir_horas` horas (seed 0029, default 4, configurable en la UI hasta "una vez al día") con dedupe `rec:{id}:{fecha}:{franja}`, sin generar avisos dentro del horario de silencio — hasta marcar hecho; con `frecuencia_dias`, completar reprograma `proxima_fecha` desde hoy y calla el resto del día. Si un recordatorio tiene `hora`, el aviso del día programado espera a esa hora.

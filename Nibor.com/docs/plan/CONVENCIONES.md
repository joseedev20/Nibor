...
- En Recordatorios (`/api/reminders`), el estado se calcula SOLO en backend y los avisos los genera únicamente la regla `recordatorios` del motor de notificaciones (dedupe por franja de minutos desde medianoche `rec:{id}:{fecha}:{franja}`, repite cada `recordatorios_repetir_minutos` minutos fuera del silencio — mínimo 5, migración 0035; el cron corre cada 5 min, wrangler.toml); el frontend nunca crea notificaciones.
...
- En Tarjetas (`/api/cards`, vista `/tarjetas`), nunca guardar el número completo, la fecha de vencimiento ni el CVV — eso sigue prohibido (validado a mano en `nombre` y bloqueado en la UI). Los **últimos 4 dígitos SÍ se permiten** (`ultimos_digitos`, migración 0036): el propio banco los muestra en cada SMS/notificación, no son un dato sensible por sí solos, y son justo lo que `widgetExpenses.js` usa para detectar sola con qué tarjeta/cuenta fue un gasto capturado por mensaje (`movements.card_id`) — si ninguna tarjeta activa coincide (o hay más de una con los mismos dígitos), el gasto se registra igual, simplemente sin tarjeta vinculada.

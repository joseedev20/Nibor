...
- En Recordatorios (`/api/reminders`), el estado se calcula SOLO en backend y los avisos los genera únicamente la regla `recordatorios` del motor de notificaciones (dedupe diario `rec:{id}:{fecha}`); el frontend nunca crea notificaciones.

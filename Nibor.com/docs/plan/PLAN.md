...
2. **Inversiones** — el reemplazo directo del Excel:
   - Tabla mensual por plataforma: saldo inicial (auto = saldo final del mes anterior), aportes acumulados, saldo base (`saldo_total_inicial`), retiros, saldo final y capital invertido histórico
...
- `/api/widget` es la API para widgets externos (Widgy, Shortcuts) con envelope JSON robusto vía `server/widgetKit.js`: `{success, data:{text,…}, error?, meta:{requestId, generatedAt}}`, timeout de 10 s a D1 y log estructurado. Hábitos y Recordatorios usan `WIDGET_TOKEN`; Gastos usa el secreto independiente `EXPENSES_SHORTCUT_TOKEN`. `GET/POST /habits` consulta y marca hábitos; `GET/POST /reminders` consulta y completa recordatorios; `GET /expenses` devuelve solo categorías de gasto y `POST /expenses` crea un movement `gasto` con `request_id` idempotente. `GET /url` permanece detrás de Access y entrega las URLs con token al usuario autenticado. En Access cada recurso tiene bypass por RUTA EXACTA: `/api/widget/habits`, `/api/widget/reminders` y `/api/widget/expenses`.

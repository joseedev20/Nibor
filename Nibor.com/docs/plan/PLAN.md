...
- `/api/widget` es la API para widgets externos (Widgy, Shortcuts) protegida por el secreto `WIDGET_TOKEN` (?token=; sin token → 404, envelope JSON en TODAS las ramas vía `server/widgetKit.js`: `{success, data:{text,…}, error?, meta:{requestId, generatedAt}}`, timeout de 10 s a D1, log estructurado por request). `GET /habits` devuelve hábitos pendientes de hoy con `text` preformateado, `resumen`, `pendientes`, `image_url` y `progreso_bucket` (0/25/50/75/100); `GET /habits?image=auto|complete|pending|0|25|50|75|100` sirve esa imagen PNG directamente (mismo path ya exento, sin bypass nuevo — el Worker lee `dist/widget-images/*.png` con el binding `ASSETS`; `auto` redondea `percent_today` al múltiplo de 25 más cercano para elegir una de 5 mascotas de progreso; token inválido → 404 vacío, error interno → pixel transparente de respaldo); `POST /habits` con `{id}` o `{nombre}` marca una unidad del hábito (misma semántica que `/habits/:id/check`, idempotente, responde `text` de confirmación). `GET /reminders` devuelve recordatorios pendientes hoy (vencidos o para hoy) con `text`, `pendientes` y `pendientes_titulos`; `POST /reminders` con `{id}` o `{titulo}` los marca hechos (misma semántica que `/reminders/:id/complete`: reprograma si tiene frecuencia, completa si es único). `GET /url` (detrás de Access) entrega ambas URLs con token al usuario autenticado. En Access el bypass es por RUTA EXACTA: `/api/widget/habits` y `/api/widget/reminders` (cada recurso nuevo necesita su propia app).

...
- `/api/widget` es la API para widgets externos (Widgy, Shortcuts) protegida por el secreto `WIDGET_TOKEN` (?token=; sin token → 404): `GET /habits` devuelve hábitos pendientes de hoy con `text` preformateado, `resumen` y `pendientes`; `POST /habits` con `{id}` marca una unidad del hábito (misma semántica que `/habits/:id/check`, idempotente al llegar al target, responde `text` de confirmación + resumen actualizado) usando la MISMA ruta para reutilizar el bypass; `GET /url` (detrás de Access) entrega la URL con token al usuario autenticado. En Access solo la ruta exacta `/api/widget/habits` tiene bypass.

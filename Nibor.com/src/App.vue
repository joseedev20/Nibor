...
const nav = [
  ...
  { to: '/recordatorios', label: 'Recordatorios', icon: 'M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z' },
  ...
  { to: '/bansky', label: 'Bansky', icon: 'M9.5 7a1.75 1.75 0 11-3.5 0 1.75 1.75 0 013.5 0zm8.5 0a1.75 1.75 0 11-3.5 0 1.75 1.75 0 013.5 0zm2.5 4.5a1.75 1.75 0 11-3.5 0 1.75 1.75 0 013.5 0zm-14 0a1.75 1.75 0 11-3.5 0 1.75 1.75 0 013.5 0zM12 11c-2.8 0-5.25 2.4-5.25 4.9 0 1.5 1.2 2.6 2.65 2.6h5.2c1.45 0 2.65-1.1 2.65-2.6 0-2.5-2.45-4.9-5.25-4.9z' },
  ...
  { to: '/tarjetas', label: 'Tarjetas', icon: 'M2.25 8.25h19.5M2.25 9h19.5m-16.5 5.25h6m-6 2.25h3m-3.75 3h15a2.25 2.25 0 002.25-2.25V6.75A2.25 2.25 0 0019.5 4.5h-15a2.25 2.25 0 00-2.25 2.25v10.5A2.25 2.25 0 004.5 19.5z' },
...
<template>
  <div class="min-h-screen bg-zinc-50 text-zinc-900 dark:bg-zinc-950 dark:text-zinc-100">
    <div
      ...
      class="fixed left-1/2 top-[calc(0.5rem+env(safe-area-inset-top))] z-[60] flex items-center gap-2 rounded-full border border-zinc-200 bg-white px-4 py-2 text-xs font-semibold text-zinc-600 shadow-lg dark:border-zinc-700 dark:bg-zinc-900 dark:text-zinc-300"
    ...
    <header class="sticky top-0 z-30 flex min-h-16 items-center justify-between border-b border-zinc-200 bg-white px-4 pt-[env(safe-area-inset-top)] md:hidden dark:border-zinc-800 dark:bg-zinc-900">
    ...
    <div
      v-if="isOffline"
      role="alert"
      class="sticky top-[calc(4rem+env(safe-area-inset-top))] z-20 flex items-center justify-between gap-3 border-b border-amber-300 bg-amber-50 px-4 py-2 text-sm text-amber-900 md:top-0 md:ml-60 dark:border-amber-900 dark:bg-amber-950 dark:text-amber-200"
    >
    ...
    >
      <div class="flex items-center gap-2 px-6 pb-5 pt-[calc(1.25rem+env(safe-area-inset-top))]">
        <div class="flex h-9 w-9 items-center justify-center rounded-xl bg-emerald-500 font-bold text-white">N</div>
        <div>
          <p class="text-sm font-semibold leading-tight">Nibor</p>
          <p class="text-xs text-zinc-500 dark:text-zinc-400">Centro personal</p>
      ...
      <nav class="mt-1 min-h-0 flex-1 space-y-1 overflow-y-auto px-3">
        <RouterLink
          v-for="item in nav"
          :key="item.to"
          :to="item.to"
